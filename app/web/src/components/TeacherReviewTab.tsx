...
export const TeacherReviewTab: React.FC<TeacherReviewTabProps> = ({
  ...
  React.useEffect(() => {
    setEditingScore(currentQuestion ? String(currentQuestion.score_value ?? "") : "");
  }, [selectedQuestionIndex, currentQuestion?.id, currentQuestion?.score_value]);

  // 分值为人工 / 原卷 / 自动 来源的可视化标记（方案 6 / 25）
  const getScoreSourceMeta = (source?: string) => {
    const map: Record<string, { label: string; className: string }> = {
      original: { label: "原卷分值", className: "bg-emerald-50 text-emerald-700 border-emerald-200" },
      manual: { label: "人工分值", className: "bg-blue-50 text-blue-700 border-blue-200" },
      auto: { label: "自动配分", className: "bg-slate-100 text-slate-600 border-slate-200" },
      ai: { label: "AI 推测", className: "bg-amber-50 text-amber-700 border-amber-200" },
    };
    return map[source || ""] || null;
  };

  // 教师手工设置满分：写入 scoreSource=manual，后续自动配分不会再覆盖（方案 25）
  const handleSaveQuestionScore = async () => {
    if (!selectedExam || !currentQuestion) return;
    const value = Number(editingScore);
    ...
    try {
      const res = await fetch(`/api/exams/${selectedExam.id}/questions/${currentQuestion.id}/score`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || "分值保存失败");
      }
      onRefreshExams();
    } catch (e: any) {
      alert(e.message || "分值保存失败");
    } finally {
      setIsSavingScore(false);
    }
  };

  const scoreSummary = selectedExam?.score_summary;
  ...
  return (
    <div className="h-[calc(100vh-105px)] flex flex-col overflow-hidden">
      ...
      {/* 自动配分结果总览：题型 / 题数 / 总分（方案 29 / 30） */}
      {scoreSummary && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm shrink-0 mb-4 overflow-hidden">
          <button
            type="button"
          ...
          >
            <span className="flex items-center gap-2 text-xs font-bold text-slate-700">
              ...
              <span className="font-medium text-slate-500">
                配置总分 {scoreSummary.configuredTotalScore} 分 · 实际合计 {scoreSummary.totalScore} 分 ·
                识别原有 {scoreSummary.fixedScore} 分 · 自动分配 {scoreSummary.autoScore} 分（{scoreSummary.autoQuestionCount} 题）
              </span>
            </span>
            <span className="text-[11px] font-bold text-blue-600">{showScoreBreakdown ? "收起" : "查看明细"}</span>
          </button>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-slate-500">
                    <th className="py-1.5 pr-3 text-left font-semibold">题型</th>
                    <th className="py-1.5 pr-3 text-right font-semibold">题目数</th>
                    <th className="py-1.5 text-right font-semibold">总分</th>
                  </tr>
                </thead>
                <tbody>
                  {scoreSummary.typeStats.map((item) => (
                    <tr key={item.key} className="border-t border-slate-100">
                      <td className="py-1.5 pr-3 text-slate-700 font-medium">{item.label}</td>
                      <td className="py-1.5 pr-3 text-right text-slate-600">{item.count}</td>
                      <td className="py-1.5 text-right font-bold text-slate-900">{item.totalScore}</td>
                    </tr>
                  ))}
                  <tr className="border-t-2 border-slate-200">
                    <td className="py-1.5 pr-3 font-bold text-slate-900">合计</td>
                    <td className="py-1.5 pr-3 text-right font-bold text-slate-900">{selectedExam?.questions?.length || 0}</td>
                    <td className="py-1.5 text-right font-extrabold text-blue-700">{scoreSummary.totalScore} 分</td>
                  </tr>
                </tbody>
              </table>
              {scoreSummary.adjustedQuestionCount > 0 && (
                <p className="flex items-center gap-1.5 text-[11px] text-amber-700">
                  <AlertCircle className="w-3 h-3" />
                  发现 {scoreSummary.adjustedQuestionCount} 道题分值经过自动调整（尾差修正），请抽查确认。
                </p>
              )}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {(() => {
                        const meta = getScoreSourceMeta(q.score_source);
                        return meta ? (
                          <span className={`text-[10px] px-1.5 py-0.5 rounded border font-bold ${meta.className}`}>
                            {meta.label}
                          </span>
                        ) : null;
                      })()}
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
                <div className="flex items-center justify-between gap-3 pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    {(() => {
                      const meta = getScoreSourceMeta(currentQuestion.score_source);
                      return meta ? (
                        <span className={`text-[11px] px-2 py-0.5 rounded-full font-bold border ${meta.className}`}>
                          {meta.label}
                        </span>
                      ) : null;
                    })()}
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        disabled={selectedExam?.status === "AUDITED"}
                        aria-label="设置题目满分"
                        className="w-16 text-xs p-1 border border-slate-300 rounded-lg text-center disabled:bg-slate-100"
                      />
                      <button
                        onClick={handleSaveQuestionScore}
                        disabled={isSavingScore || selectedExam?.status === "AUDITED"}
                        className="flex items-center gap-1 px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-[11px] rounded-lg transition disabled:opacity-50"
                      >
                        <Save className="w-3 h-3" />
                        {isSavingScore ? "保存中" : "改分"}
                      </button>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
